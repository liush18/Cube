    ...
    int mode)
{
    ...
    userRTKInit(&rtk,popt,userPPPnx_p(popt));
    while ((nobs=userInputObs(obs,rtk.sol.stat,popt))>=0) {
        ...
        if (!userRTKPos_p(&rtk,obs,n,nav)) continue;
    ...
    const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt)
{
    ...
    if (popt->ionoopt==IONOOPT_IFLC) sprintf(soltype,"if");
    else sprintf(soltype,"uu");
    if (popt->modear>ARMODE_OFF) sprintf(soltype+2,"_fixed");
    else sprintf(soltype+2,"_float");
    ...
    for (i=0;i<n;i++) {
        ...
        if (ext=strrchr(outfile,FILEPATHSEP)) 
            *ext='\0';
        if (ext=strrchr(outfile,FILEPATHSEP)) 
            sprintf(ext+1,"%s%s","ppp_solution_",soltype);
        ...
        if (sopt->sstat>0) {
            ...
            logRclkClose_p();
            logRclkOpen_p(statfile,popt);
            /* open ztd record file */
            strcpy(statfile,outfile);
            strcat(statfile,".ztd");
            logZtdClose_p();
            logZtdOpen_p(statfile,popt);
            /* open ztd record file */
            strcpy(statfile,outfile);
            strcat(statfile,".ion");
            logIonClose_p();
            logIonOpen_p(statfile,popt);
            /* open ambiguity record file */
            strcpy(statfile,outfile);
            strcat(statfile,".amb");
            logAmbClose_p();
            logAmbOpen_p(statfile,popt);
            /* open residual record file */
            strcpy(statfile,outfile);
            strcat(statfile,".res");
            logResClose_p();
            logResOpen_p(statfile,popt);
        }
    ...
    logRclkClose_p();logResClose_p();logZtdClose_p();logIonClose_p();
    logAmbClose_p();traceclose();
