...
* return : none
* notes  : (0:time,1:sbias,2:trop,3:amb,4:rbias)
* ----------------------------------------------------------------------------*/
	...
	int frq, int trp, int datum, double value, int lastEp)
{
	...
	else if (type==TYPE_ION) {
		/* #ION,rcv name,satid,value */
		satno2id(sat,satid);
		fprintf(fpr1,"%s %s %s %.6f\n",CST_STR_ION,rcv,satid,value);
	}
	...
	else if (type==TYPE_SCB) {
		/* #SCB,satid,frequency number,value */
		satno2id(sat,satid);
		fprintf(fpr1,"%s %s %d %.6f\n",CST_STR_SCB,satid,frq,value);
	}
	else if (type==TYPE_RCB) {
		/* #RCB,rcv name,frequency number,value */
		fprintf(fpr1,"%s %s %d %.6f\n",CST_STR_RCB,rcv,frq,value);
	}
	else if (type==TYPE_SPB) {
		/* #SPB,satid,frequency number,value */
		satno2id(sat,satid);
		fprintf(fpr1,"%s %s %d %.6f\n",CST_STR_SPB,satid,frq,value);
	}
	else if (type==TYPE_RPB) {
		/* #RPB,rcv name,frequency number,value */
		fprintf(fpr1,"%s %s %d %.6f\n",CST_STR_RPB,rcv,frq,value);
	}
	fflush(fpr1);
}
...
static int getRecordIndex(net_t *uck)
{
	...
	while (fgets(buff,sizeof(buff),fpg1)) {
		...
		else if (!strncmp(CST_STR_ION,buff,4)) {
			if (sscanf(buff+5,"%s %s %lf",uck->csd[uck->nc].rcv,satid,
				&uck->csd[uck->nc].value)<2) {

				trace(2,"decode %s error %s\n",CST_STR_ION,buff);
				return 0;
			}
			uck->csd[uck->nc].sat=satid2no(satid);
			uck->csd[uck->nc].type=TYPE_ION;
		}
		...
		else if (!strncmp(CST_STR_SCB,buff,4)) {
			if (sscanf(buff+5,"%s %d %lf",satid,&uck->csd[uck->nc].frq,
				&uck->csd[uck->nc].value)<3) {
				trace(2,"decode %s error %s\n",CST_STR_SCB,buff);
				return 0;
			}
			uck->csd[uck->nc].type=TYPE_SCB;
			uck->csd[uck->nc].sat=satid2no(satid);
		}
		else if (!strncmp(CST_STR_RCB,buff,4)) {
			if (sscanf(buff+5,"%s %d %lf",uck->csd[uck->nc].rcv,
				&uck->csd[uck->nc].frq,&uck->csd[uck->nc].value)<3) {
				trace(2,"decode %s error %s\n",CST_STR_RCB,buff);
				return 0;
			}
			uck->csd[uck->nc].type=TYPE_RCB;
		}
		else if (!strncmp(CST_STR_SPB,buff,4)) {
			if (sscanf(buff+5,"%s %d %lf",satid,&uck->csd[uck->nc].frq,
				&uck->csd[uck->nc].value)<3) {
				trace(2,"decode %s error %s\n",CST_STR_SPB,buff);
				return 0;
			}
			uck->csd[uck->nc].type=TYPE_SPB;
			uck->csd[uck->nc].sat=satid2no(satid);
		}
		else if (!strncmp(CST_STR_RPB,buff,4)) {
			if (sscanf(buff+5,"%s %d %lf",uck->csd[uck->nc].rcv,
				&uck->csd[uck->nc].frq,&uck->csd[uck->nc].value)<3) {
				trace(2,"decode %s error %s\n",CST_STR_RPB,buff);
				return 0;
			}
			uck->csd[uck->nc].type=TYPE_RPB;
		}
		uck->nc++;
	}
...
extern int getRecord_u(net_t *uck, const char *fileInd, const char *fileVar)
{
	...
	*p='\0';
	...
	for (i=0;i<nc;i++) {
		if (csd[i].type==TYPE_TRP) {
			...
			crcv[r-1]|=1;
			ind[i]=UIT(r,opt)+csd[i].trp;
		}
		else if (csd[i].type==TYPE_ION) {
			if (!(p=strstr(sta,csd[i].rcv))) continue;
			r=(int)((p-sta)/4+1);
			crcv[r-1]|=1;
			ind[i]=UII(r,csd[i].sat,opt);
		}
		else if (csd[i].type==TYPE_AMB) {
			if (!(p=strstr(sta,csd[i].rcv))) continue;
			r=(int)((p-sta)/4+1);
			crcv[r-1]|=1;
			ind[i]=UIAMB(r,csd[i].sat,csd[i].frq,opt);
		}
		else if (csd[i].type==TYPE_SCB) {
			ind[i]=UISCB(csd[i].sat,csd[i].frq,opt);
		}
		else if (csd[i].type==TYPE_RCB) {
			if (!(p=strstr(sta,csd[i].rcv))) continue;
			r=(int)((p-sta)/4+1);
			crcv[r-1]|=1;
			ind[i]=UIRCB(r,csd[i].frq,opt);
		}
		else if (csd[i].type==TYPE_SPB) {
			ind[i]=UISPB(csd[i].sat,csd[i].frq,opt);
		}
		else if (csd[i].type==TYPE_RCB) {
			if (!(p=strstr(sta,csd[i].rcv))) continue;
			r=(int)((p-sta)/4+1);
			crcv[r-1]|=1;
			ind[i]=UIRPB(r,csd[i].frq,opt);
		}
		else {
	...
	for (i=0;i<nd;i++) {
		...
		ind[i]=UIAMB(r,dtm[i].sat,dtm[i].frq,opt);
	}
...
{
	net_t uck;
	char str[]="C:\\Users\\Lenovo\\Desktop\\033\\solution1\\apm21432.clk.index";
	char str2[]="C:\\Users\\Lenovo\\Desktop\\033\\solution1\\apm21432.clk.value";
	uck.nc=uck.ncmax=uck.nd=uck.ndmax=0; uck.csd=NULL; uck.dtm=NULL;
	getRecord_u(&uck,str,str2);
}